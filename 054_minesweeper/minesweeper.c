...
board_t * makeBoard(int w, int h, int numMines) {
  //WRITE ME!
  //return NULL;
  board_t * Bptr = malloc(sizeof(*Bptr));
  Bptr->width = w;
  Bptr->height = h;
  Bptr->totalMines = numMines;
  Bptr->board = malloc(Bptr->height * sizeof(*Bptr->board));
  for (int y = 0; y < Bptr->height; y++) {
    Bptr->board[y] = malloc(Bptr->width * sizeof(*Bptr->board[y]));
    for (int x = 0; x < Bptr->width; x++) {
      Bptr->board[y][x] = UNKNOWN;
    }
  }
  for (int cnt = 0; cnt < Bptr->totalMines; cnt++) {
    addRandomMine(Bptr);
  }
  return Bptr;
}
...
int countMines(board_t * b, int x, int y) {
  //WRITE ME!
  //return 0;
  int x_min = (x - 1 > 0) ? x - 1 : 0;
  int x_max = (x + 1 < b->width - 1) ? x + 1 : b->width - 1;
  int y_min = (y - 1 > 0) ? y - 1 : 0;
  int y_max = (y + 1 < b->height - 1) ? y + 1 : b->height - 1;
  int mine_cnt = 0;
